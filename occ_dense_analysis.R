...
# Iterate through RUs
years <- 1:24
colors <- c("raven trend" = "#994F00", "tortoise trend" = "#006CD1")
for (ru in rus_list) {
  ...
  df <- data.frame(
    year = years + 2000,
    y_cc = years * m_cc + b_cc,
    y_ga = years * m_ga + b_ga
  )
  
  # Plot fits
  plot <- ggplot(df, aes(x = year)) +
    geom_line(aes(y = y_cc, color = "raven trend")) + 
    geom_line(aes(y = y_ga, color = "tortoise trend")) +
    scale_color_manual(values = colors, name = "species") +
    ylab("presence variable") +
    ggtitle(paste0(
      "Tortoise density and raven occupancy trends for ",
      str_replace(ru, "_", " ")
    )) +
    theme(
      panel.background = element_rect(fill = "white"),
